...
export default function WelcomeBanner() {
  ...
  <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-violet-background/80 backdrop-blur-md animate-in fade-in duration-500">
    ...
    <div className="relative w-full max-w-sm flex flex-col items-center">
      ...
      <div className="w-full bg-gradient-to-b from-violet-background via-violet-background to-black text-white p-8 rounded-bigview relative overflow-hidden shadow-[0_0_50px_rgba(139,92,246,0.2)] border border-white/10 animate-in zoom-in-95 duration-300">
        ...
        <div className="absolute top-0 right-0 w-32 h-32 bg-violet-glow/20 rounded-full blur-[60px] -mr-10 -mt-10" />
        ...
        <div className="relative z-10 flex flex-col items-center text-center">
          ...
          <div className="space-y-3">
            ...
            <p className="text-white/40 text-[11px] font-black uppercase tracking-[0.2em]">
              Account Secured via <span className="text-gold-buttons">Privy</span>
            </p>

            <div className="h-px w-12 bg-white/10 mx-auto my-4" />

            <p className="text-white/70 text-sm font-medium leading-relaxed opacity-90 uppercase tracking-tight">
              You're now live on <span className="text-white font-black italic">Base Sepolia</span>. Start staking to accumulate <span className="text-gold-buttons font-black italic">BVW</span> governance rewards automatically.
            </p>

            <div className="pt-6 w-full flex flex-col gap-2">
              <button 
                onClick={closeBanner}
                className="w-full py-4 bg-gold-buttons text-text-color rounded-bigview font-black text-sm uppercase tracking-widest hover:opacity-90 transition-all shadow-lg shadow-gold-buttons/10 active:scale-95 flex items-center justify-center gap-2"
              >
      ...
      <button 
        onClick={closeBanner} 
        className="mt-8 p-4 bg-white/5 hover:bg-white/10 rounded-full border border-white/10 transition-all group backdrop-blur-md"
      >
